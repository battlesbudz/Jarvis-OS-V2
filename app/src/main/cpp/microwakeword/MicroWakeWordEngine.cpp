...
bool MicroWakeWordEngine::processFeatureFrame(const int8_t* features) {
    ...
    if (interpreter_->Invoke() != kTfLiteOk) {
        throw std::runtime_error("microWakeWord inference failed");
    }
    ...
    uint32_t sum = 0;
    for (auto value : recentProbabilities_) sum += value;
    chunkProbability_ = std::max(chunkProbability_, static_cast<float>(sum) / (255.0f * slidingWindowSize_));
