...
These bands are a transparent resource screen, **not empirically measured fit boundaries**. The internal risk separates 85%+ as especially tight. Bundles can include memory-mapped embeddings and optional encoders; quantization, cache allocation and architecture change actual working memory. No new numeric RAM-footprint or speed estimates are invented. On a 12 GiB test profile, at least 85 of the 90 current models stay below the tight-headroom bands, including E2B and E4B. Qwen3-8B also passes this size-only screen, but a reported Fold6 startup failure now overrides its positive presentation. Gemma 26B/31B and Qwen3-14B remain visible and selectable with more demanding memory qualifiers.
...
### Reported startup failures

On 2026-09-19, Justin reported that Qwen3-8B would not start on the Galaxy Z Fold6. Its card and selected-model summary now prominently show “Not recommended · reported startup failure,” regardless of installed/tested status or free RAM. It is excluded from starting-option eligibility but stays in size order and can still be selected for troubleshooting. The underlying memory comparison is retained only as an explicitly labeled size-only estimate in expanded detail. This report does not establish an out-of-memory cause, a fix, or failure on other phone families.

The report is scoped to SM-F956 devices and the current Qwen3-8B GPU artifact (SHA-256 `cb4e6d0de4bbf6656d177812cf0c6a983967dedd17e7f88e84b901c3a9862a42`). A replacement bundle/backend must be assessed separately. Regression checks cover positive-label suppression, resource independence, artifact/backend/device scoping, adjacent models and retained catalog availability.
...
15 focused model JVM tests passed locally for this correction. They cover catalog pinning, family ordering, suitability guidance, and the reported Qwen3-8B failure's artifact/backend/device scope. Android release compilation, the full JVM suite, native packaging, signing and APK assembly run in the existing PR workflow. Actual startup causes, model speed, UI rendering and newly listed model behavior still require device testing.
