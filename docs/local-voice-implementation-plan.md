...
Status: in progress (2026-09-10). Build 650 validated immediate follow-up handoff and readiness diagnostics. Bounded natural-correction recognition is now implemented for eligible warm Moonshine sessions; Android debug/release validation passed in build 653, but phone evidence exposed natural-path availability failures. Recovery, prompt result consumption and pre-reply stream reservation are implemented below; replacement-build and device acceptance are pending. Keyword fallback remains available; route/AEC and sustained native-load acceptance are still open.
