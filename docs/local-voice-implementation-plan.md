...
Validation: 94 focused Kotlin/JUnit tests pass with assertions enabled, including 18 new tests. They cover exact-once onset replay, no ASR work on silence, echo/backchannel rejection, synthetic sustained input with bounded probe count, stale worker results, worker failure/close failure, playback pressure, exclusive native ownership, keyword progress during blocked native decoding, warm-only admission, later app-target corrections and cancellation preservation. Android debug/release validation is pending. Synthetic workload tests establish queue/ownership behavior; they do not establish Fold 6 RTF, acoustic echo cancellation or audibility.
