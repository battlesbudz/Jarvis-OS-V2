...
Examples include “Read battery, then set media volume to 30 percent, then open Settings.” The same action plan is used for completed text requests and completed voice transcripts. An immediate “Yes” or “open it” confirmation can resolve a specific app offer/request from the latest turn; stale history is ignored. Voice additionally requires every planned clause to pass the final-transcript guard and agree with the confirmed request. Images and audio attachments are ordinary model turns and do not enter this action planner.
...
| Ordered batches, strict mismatch rejection, explicit repeats, replay deduplication, failure stop | `ActionTurnRunnerTest` | JVM | Included in repaired focused host run (37 tests total) |
| Immediate app confirmation and final voice agreement | `ActionTurnRunnerTest.authoritativePlanKeepsImmediateOffersButRejectsStaleHistory`; `FinalVoiceToolGuardTest` | JVM logic | Added and included in 37-test focused run; real voice/model remains unverified |
...
Real model weights, physical audio capture, and Fold 6 performance are outside emulator evidence. A simulated model output plus a real Android executor is an executor check, not a real-model tool-selection result.
