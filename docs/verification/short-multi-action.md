...
The Android journeys use simulated `ToolCall` objects and the real `AndroidMobileActionExecutor`. Build 759 historically passed `test14`–`test19` on both API variants, verifying Android battery state, media-volume state, Settings launch, invalid-plan preservation, partial failure, natural routing, and duplicate suppression. They do not verify real model weights, real model tool selection, physical Fold 6 behavior, or acoustic voice capture. The candidate has a 102-check focused host result; its four new Android journeys (`test20`–`test23`) remain pending CI.

## Test map

| Check | Test | Level | Status / evidence |
| --- | --- | --- | --- |
| Three ordered actions: battery → volume → Settings | `ReleaseJourneyTest.test14_threeActionTurnUsesRealAndroidState` | Android instrumentation; simulated model output + real executor | Build 759 historical pass on both variants |
| Natural Settings request opens then reads battery without lookup | `ReleaseJourneyTest.test18_naturalActionRoutingOpensSettingsThenReadsBattery` | Android instrumentation; controlled calls + real executor | Build 759 historical pass on both variants |
| Retry-prefixed literal `fistbook` fails before battery; Facebook substitution rejected | `ReleaseJourneyTest.test19_retryLiteralUnknownAppStopsWithoutBattery` | Android instrumentation; controlled calls + real executor | Build 759 historical pass on both variants |
| Invalid plan leaves volume unchanged | `ReleaseJourneyTest.test15_invalidActionTurnPreservesAndroidState` | Android instrumentation; real executor | Build 759 historical pass on both variants |
| Failed app stops later steps while completed volume remains | `ReleaseJourneyTest.test16_partialFailureKeepsCompletedAndroidAction` | Android instrumentation; real executor | Build 759 historical pass on both variants |
| Cancellation stops before the second action; completed receipt remains visible and durable; duplicate model pass does not replay completed volume | `ReleaseJourneyTest.test17_cancelledAndDuplicateActionTurnsDoNotReplay`; `ActionTurnRunnerTest.actualCoroutineCancellationAfterReceiptSkipsLaterAction`; `ActionTurnRunnerTest.actualCancellationBeforeFirstEffectHasNoReceipt`; `ActionTurnRunnerTest.cancellationAndRetryCapNeverReplayCompletedEffects` | Android/JVM | Build 759 historical Android pass; JVM coverage retained |
...
The candidate accepted-action mode keeps one runtime-owned FIFO worker and a call-owned ASR/listening pump. It admits at most three outstanding requests total (running, queued, or awaiting result reporting), with one to three existing actions per request, and runs requests in admission order. Overflow receives an explicit not-accepted response. Only `read_battery`, `set_volume`, and `open_app` may enter the existing native `ConversationRuntime`/`ActionTurnRunner` path. The queue freezes the validated plan and source text at admission, retains completed executor receipts, and deduplicates replayed callbacks by stable utterance ID; a fresh utterance repeating the same words remains a new request.

Ordinary barge-in, `stop`, `stop speaking`, `wait`, and `hold on` silence the current speech attempt while accepted tasks continue. `cancel that action`, `cancel the current action`, and `cancel all actions` are explicit scoped controls; they stop targeted unfinished work and preserve completed receipts. Cancellation cannot undo a synchronous Android operation already begun. End call detaches speech delivery while bounded accepted tasks may finish into the original saved record; a fresh report attempt can retry undelivered results without rerunning actions, and it does not auto-replay work in a new call.
...
`test20_followupQueuesWhileAcceptedAndroidActionRuns`, `test21_speechInterruptionPreservesAcceptedAndroidActions`, `test22_explicitCancellationKeepsCompletedAndroidReceipts`, and `test23_actionResultsSurviveEndCallWithoutReplay` are added sources pending Android CI. They use controlled native `ToolCall` outputs and the real Android executor; they do not verify Gemma weights, ASR accuracy, acoustic barge-in, Bluetooth, thermal behavior, or physical Fold 6 performance. Candidate `34c0c965` passed source review with no blocking findings and covers fresh report retry and durable ended-call outcomes without replay in focused tests. Exact Android CI is pending.

## SHORT Fold 6 checklist

Use a physical Fold 6 only as a product check after the continuous-action Android CI evidence is available. Build 759’s Facebook-plus-battery success is historical real-device evidence for the earlier path, not proof of continuous listening. Record the exact app revision and model state.
