...
4. Execute twenty-six named release instrumentation scenarios, then kill/relaunch Jarvis in a separate process to check persisted selection. Retain screenshots, UI hierarchy, Android logs, test output, package metadata, APK hashes, source SHA and PR head.
...
Build 759 is historical verified evidence: its receipt recorded 582 JVM tests and 20/20 journeys on both API 30 normal and API 35 compact, with APK publication and consolidation passed. That run includes the natural-routing repair but does not verify continuous accepted-action listening. The current continuous-action snapshot has a 102-check focused host result and four added journey sources (`test20`–`test23`), with Android CI still pending.

Continuous accepted-action voice is a separate candidate workstream with Android verification pending. The proposed runtime-owned FIFO admits up to three outstanding requests (running, queued, or awaiting result reporting), each with one to three existing actions, and serializes native work through the existing `ConversationRuntime`/`ActionTurnRunner` path. The accepted follow-up forms include “But actually can you open up youtube for me” and “And then open up Facebook after that”. Conditions such as “after the download finishes” remain rejected, and overflow receives an explicit not-accepted response. Ordinary barge-in, `stop`, and `stop speaking` silence the current TTS attempt while preserving accepted work. Explicit scoped cancellation targets unfinished work by task/call ID; it may prevent later steps but cannot undo a synchronous Android operation already begun. Completed executor receipts remain durable, FIFO reports are retained, duplicate utterance IDs do not execute twice, and only `read_battery`, `set_volume`, and `open_app` are supported.

Candidate `34c0c965` passed source review with no blocking findings and reports 102 focused checks, including ordered accepted work surviving speech interruption, scoped action cancellation, queued follow-ups, fresh report retry, and durable ended-call outcomes. Full exact Android release CI remains pending. The four new Android journey sources (`test20`–`test23`) use controlled native `ToolCall` outputs with the real Android executor and are pending CI; they are not real Gemma, ASR, acoustic, or physical-device evidence. No simultaneous text-during-call UI expansion or model replacement is implied; existing model support and the noFunctionGemma boundary remain preserved.
