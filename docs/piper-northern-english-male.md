...
Uses Sherpa VITS with speaker 0, 22,050 Hz output, bundled espeak data and normal 1x playback by default. Explicit test/call pace profiles still apply. Existing Kokoro and Paul selections remain available.

The first test or call downloads the checksum-pinned 67,210,490-byte upstream archive; subsequent use is offline, with no account or key. Installation verifies every archived file size and retains MODEL_CARD. Dataset attribution: OpenSLR 83, CC BY-SA 4.0 International, http://www.openslr.org/83/.
...
This is a bounded voice audition under B1; it does not replace Paul or upgrade Kokoro.
...
Diagnostics identify `piper_whole_passages_max640_v1`, actual submission count in `phrases`, and per-passage character counts in retained benchmark boundary traces. The unrelated Paul stability label is now unavailable for Piper. Tests cover short-reply preservation, story grouping, bounded long/run-on input without text loss, profile persistence, native sentence batching, and natural-pause configuration. Device acceptance remains open.
